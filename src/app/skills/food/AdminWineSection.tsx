...
/* ── Delete button (used inline on each DB wine card) ─────── */
function DeleteWineButton({ wineId }: { wineId: string }) {
  ...
  async function handleDelete() {
    setBusy(true)
    await fetch(`/api/admin/wines/${wineId}`, { method: 'DELETE' })
    router.refresh()
  }
  ...
  if (confirming) return (
    <div style={{ display: 'flex', gap: 4 }}>
      ...
      <button onClick={() => setConfirming(false)} style={{ padding: '4px 8px', background: 'rgba(0,0,0,0.4)', border: '1px solid rgba(200,155,60,0.2)', color: S.text3, fontSize: 11, borderRadius: 4, cursor: 'pointer', fontFamily: 'Inter, sans-serif' }}>
        Cancel
      </button>
    </div>
  )
...
/* ── Add wine form ─────────────────────────────────────────── */
function AddWineForm({ onAdded }: { onAdded: () => void }) {
  ...
  return (
    <form onSubmit={handleSubmit} style={{ background: '#0e0b14', border: '1px solid rgba(200,155,60,0.3)', borderRadius: 14, padding: '22px 20px', display: 'flex', flexDirection: 'column', gap: 16 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <span style={{ fontFamily: "'Press Start 2P', monospace", fontSize: 8, color: S.gold }}>🍷 ADD WINE</span>
      ...
      <div>
        ...
        {imagePreview && (
          <button type="button" onClick={() => { setImageData(null); setImagePreview(null) }} style={{ marginTop: 6, background: 'none', border: 'none', color: S.text3, fontSize: 12, cursor: 'pointer', fontFamily: 'Inter, sans-serif' }}>
            Remove image
          </button>
        )}
      </div>
      ...
      <div>
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
          {WINE_TAGS.map(t => (
            <button key={t} type="button" onClick={() => setTag(t)} style={{ padding: '6px 12px', borderRadius: 6, cursor: 'pointer', fontFamily: "'Press Start 2P', monospace", fontSize: 5.5, border: `1px solid ${tag === t ? 'rgba(200,155,60,0.6)' : 'rgba(200,155,60,0.18)'}`, background: tag === t ? 'rgba(200,155,60,0.12)' : 'transparent', color: tag === t ? S.gold : S.text3 }}>
              {t}
            </button>
          ))}
        </div>
      ...
      <div>
        ...
        <textarea value={notes} onChange={e => setNotes(e.target.value)} rows={3} maxLength={500} placeholder="Describe the aromas, palate, and finish..." style={{ ...inputStyle, resize: 'vertical' }} />
      </div>

      {/* Pairings */}
      <div>
        <label style={labelStyle}>FOOD PAIRINGS</label>
        <input value={pairings} onChange={e => setPairings(e.target.value)} maxLength={300} placeholder="🥩 Ribeye, 🧀 Cheese, 🍝 Pasta (comma-separated)" style={inputStyle} />
      </div>
...
/* ── Exported wrapper — two modes ─────────────────────────── */
export default function AdminWineSection({ wineId, deleteOnly }: { wineId?: string; deleteOnly?: boolean }) {
  const router = useRouter()
  if (deleteOnly && wineId) {
    return (
      <div style={{ position: 'absolute', bottom: 12, left: 12, zIndex: 4 }}>
        <DeleteWineButton wineId={wineId} />
      </div>
    )
  }
  return <AddWineForm onAdded={() => router.refresh()} />
}
