...
export default function FoodPostForm() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const res = await fetch('/api/posts', {
      ...
      body: JSON.stringify({ skill: 'FOOD', title: title.trim(), body: body.trim() }),
    })
    if (res.ok) {
    ...
    return (
      <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
        <div
          ...
          style={{
            flex: 1, background: S.card, border: `1px solid rgba(200,155,60,0.16)`,
            padding: '14px 18px', cursor: 'text',
            color: S.placeholder, fontFamily: 'Inter, sans-serif', fontSize: 15,
          }}
        >
          Start a food discussion or ask a question...
        </div>
