...
export default async function CoolPeoplePage() {
  const skillMeta = getSkillBySlug('cool-people')!
  ...
  try {
    const [communityData, postRows, rawCards] = await Promise.all([
      ...
      (prisma as any).businessCard.findMany({
        include: {
          user: { select: { id: true, username: true, skills: { select: { xp: true } } } },
        },
        orderBy: { createdAt: 'asc' },
      }),
    ])
