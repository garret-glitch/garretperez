...
const blank: FormState = { itemName: '', description: '', category: '', link: '', priceRange: '', accentColor: '#c89b3c' }
...
export default function CoolItemsSection({
  items,
  userId,
  isAdmin,
}: {
  ...
  async function handleDelete(id: string) {
    ...
    try {
      await fetch(`/api/cool-items/${id}`, { method: 'DELETE' })
    ...
    <div>
          ...
          {userId ? (
            <button
              ...
              style={{
                ...
                background: showForm ? 'transparent' : 'linear-gradient(135deg, #c89b3c, #a07828)',
                color: showForm ? '#907848' : '#0a0600',
                border: showForm ? '1px solid #3a2a18' : 'none',
                padding: '8px 14px', borderRadius: 4, cursor: 'pointer',
                boxShadow: showForm ? 'none' : '0 2px 10px rgba(200,155,60,0.3)',
              }}
            >
              {showForm ? '✕ Cancel' : '+ Share Item'}
            </button>
          ) : (
            <a href="/login" style={{
              fontFamily: "'Press Start 2P', monospace", fontSize: 7,
              background: 'linear-gradient(135deg, #c89b3c, #a07828)',
              color: '#0a0600', padding: '8px 14px', borderRadius: 4,
              textDecoration: 'none',
            }}>
              Login to Share
            </a>
        ...
        <div style={{
          ...
          <form onSubmit={handleSubmit}>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(220px, 1fr))', gap: 14, marginBottom: 18 }}>
              ...
              <Field label="Why it&apos;s cool">
                <input value={form.description} onChange={set('description')} maxLength={300} placeholder="Short reason (optional)" style={inputCss} />
              </Field>
              <Field label="Category">
                <select value={form.category} onChange={set('category')} style={{ ...inputCss, appearance: 'none' as any }}>
                  <option value="">— Pick a category —</option>
                  {CATEGORIES.map(c => <option key={c} value={c}>{c}</option>)}
                </select>
              </Field>
              <Field label="Price Range">
                <select value={form.priceRange} onChange={set('priceRange')} style={{ ...inputCss, appearance: 'none' as any }}>
                  <option value="">— Optional —</option>
                  {PRICE_RANGES.map(p => <option key={p} value={p}>{p}</option>)}
                </select>
              </Field>
              <Field label="Link (optional)">
                <input value={form.link} onChange={set('link')} maxLength={300} placeholder="amazon.com/... or any link" style={inputCss} />
              </Field>
            </div>
            ...
            <div style={{ marginBottom: 16 }}>
              ...
              <div style={{ maxWidth: 320 }}>
                <CoolItemCard
                  item={{
                    id: 'preview',
                    itemName: form.itemName || 'Item Name',
      ...
      {items.length === 0 && !showForm && (
        <div style={{
          ...
          <p style={{ fontFamily: "'Press Start 2P', monospace", fontSize: 8, color: '#f0d898', marginBottom: 8 }}>No items yet</p>
          <p style={{ fontFamily: "'Inter', system-ui, sans-serif", fontSize: 13, color: '#907848', marginBottom: 20 }}>
            Share a cool item the community should know about.
          </p>
          {userId && (
            <button onClick={openForm} style={{ fontFamily: "'Press Start 2P', monospace", fontSize: 7.5, background: 'linear-gradient(135deg, #c89b3c, #a07828)', color: '#0a0600', border: 'none', padding: '10px 18px', borderRadius: 4, cursor: 'pointer' }}>
              + Be the First
            </button>
          )}
        </div>
